...
function processPost(post: Post) {
  ...
  return (
    <li key={id} className="mb-4">
      <Link
        href={`/community-motivation/forums/${id}`}
  ...
  return (
    <div className="w-full max-w-2xl mb-6">
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4 flex justify-between items-center">
        <Link
          href={`/community-motivation/forums?page=${currentPage - 1}`}
...
export default async function ForumsPage(props: { searchParams: BlogPageProps }) {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center p-10">
      <h1 className="text-6xl font-extrabold tracking-tight">Forums & Support Groups</h1>
